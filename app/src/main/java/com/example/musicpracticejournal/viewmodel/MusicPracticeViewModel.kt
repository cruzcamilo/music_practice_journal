...
class MusicPracticeViewModel(private val repository: MusicPracticeRepository): ViewModel() {
    ...
    val allMusicFragments : LiveData<List<MusicFragment>> = repository.allMusicFragments.asLiveData()
    val emptyMessageVisibility = MutableLiveData(allMusicFragments.value?.size == 0)
